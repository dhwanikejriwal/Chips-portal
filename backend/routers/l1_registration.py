...
async def submit_l1_registration(
    ...
    try:
        ...
        station_req = db.query(StationIDRequest).filter(
            ...
            StationIDRequest.status == "approved"
        ).first()
...
async def get_l1_requests(
    ...
    base_query = """
        SELECT r.id, r.request_code, r.station_id, r.model_type, CAST(r.status AS TEXT) as raw_status, r.created_at, r.updated_at, d.district_name as dist_name
    ...
    for row in query_exec.fetchall():
        compiled_list.append({
            ...
            "reviewed_at": str(row.updated_at)[:19] if str(row.raw_status).upper() in ["APPROVED", "REVIEWED", "REVERTED", "REJECTED"] else None,
