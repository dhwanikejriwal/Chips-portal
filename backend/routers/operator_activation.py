...
def submit_operator_activation(
    ...
    if nseit_req and nseit_req.status in ["Approved", "Skipped"]:
        nseit_done = True
    ...
    db.add(new_request)
    db.flush()
    new_request.request_no = candidate.request_code
    db.flush()
...
def get_dc_requests(dc_id: int, db: Session = Depends(get_db)):
    """All requests for the DC's district — shown on DC portal list page.

    Scope by the coordinator's district, not by the individual dc_id that
    created each request: a district can have more than one DC/EDM login, and
    every coordinator must see (and act on) all of the district's requests —
    including anything CHiPS reverted or rejected. Falls back to dc_id when the
    user's district cannot be resolved.
    """
    ...
    requests = query.order_by(OperatorActivationRequest.submitted_at.desc()).all()
