...
def _base_filters(q, frm, to, districts, stations, ea_codes, search, off_hours,
    ...
    if districts:
        q = q.filter(ODA.machine_district.in_(districts))
    if stations:
...
def filters(db: Session = Depends(get_db)):
    districts = [d[0] for d in db.query(ODA.machine_district)
                 .filter(ODA.machine_district.isnot(None)).distinct()
                 .order_by(ODA.machine_district).all()]
...
def operator_profile(session_operator_id: str, db: Session = Depends(get_db)):
    ...
    return {
        "session_operator_id": m.session_operator_id,
        "operator_name": (kt.operator_name_raw if kt and kt.operator_name_raw else m.operator_name),
        "model": model,
