...
def _rel_to_dicts(rel):
    """Safely convert a DuckDB query relation to a list of dicts, using Arrow if available."""
    try:
        return rel.to_arrow_table().to_pylist()
    except Exception:
...
def transform_file(
    ...
    try:
        ...
        rejected = _rel_to_dicts(con.execute(f"""
            SELECT * FROM base
        ...
        fact_rows = _rel_to_dicts(con.execute(f"""
        ...
        station_rows = _rel_to_dicts(con.execute("""
        ...
        multi = _rel_to_dicts(con.execute("""
            SELECT station_ea_code, station_number,
