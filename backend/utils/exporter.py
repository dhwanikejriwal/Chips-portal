...
def generate_csv_export(query_data, column_mappings: dict, filename_prefix: str, ids: str = None):
    """
    ...
    # Pre-parse ids filter if provided
    id_list = [str(i).strip() for i in ids.split(',')] if ids else None
    
    for record in query_data:
        # Check if record passes the ID filter (using r_id or similar primary keys)
        if id_list is not None:
            rec_id = getattr(record, 'r_id', None)
            if not rec_id and isinstance(record, dict):
                rec_id = record.get('r_id') or record.get('id')
            if rec_id is None or str(rec_id) not in id_list:
                continue
    ...
    # Use utf-8-sig to include BOM so Excel opens it properly
...
def generate_excel_export(query_data, column_mappings: dict, filename_prefix: str):
    """
    Universal reusable function to export any database model dataset to Excel.
    """
    extracted_rows = []
