...
(function () {
  ...
  function renderTable(data) {
    ...
    $("oaTbody").innerHTML = rows.map(function (r) {
      ...
      tds.push('<td class="oa-freeze oa-mono">' + txt(r.session_operator_id) + '</td>');
      ...
      tds.push('<td>' + txt(r.machine_district) + '</td>');
  ...
  var ANOM_COLS = [
    ["session_operator_id", "Operator ID", "oa-freeze oa-mono", false],
    ...
    ["model", "Model", "", false],
    ["kit_tracker_operator", "Kit Tracker Operator", "oa-mono", false],
  ...
  function renderAnomalies(data) {
    ...
    $("oaAnomTbody").innerHTML = data.rows.map(function (r) {
      var tds = ANOM_COLS.map(function (c) {
        ...
        return '<td class="' + c[2] + '">' + (v === null || v === undefined || v === "" ? "—" : esc(v)) + '</td>';
      });
      return '<tr tabindex="0" role="button" data-sid="' + encodeURIComponent(r.session_operator_id) + '">' + tds.join("") + '</tr>';
    }).join("");
  ...
  function loadFilters() {
    return api("filters").then(function (f) {
      ...
      sel.innerHTML = '<option value="">All districts</option>' +
        f.districts.map(function (d) {
          var esc = String(d).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/"/g, "&quot;");
          return '<option value="' + esc + '"' + (current === d ? " selected" : "") + '>' + esc + '</option>';
        }).join("");
      // constrain date pickers to data bounds
      if (f.minDate) { $("oaFrom").min = f.minDate; $("oaTo").min = f.minDate; }
      if (f.maxDate) { $("oaFrom").max = f.maxDate; $("oaTo").max = f.maxDate; }
    });
  }
  ...
  function loadReminder() {
    api("missing-dates").then(function (m) {
      ...
      if (m.count > 0) {
        el.hidden = false;
        var shown = m.dates.slice(-8).map(fmtDate).join(", ");
        el.innerHTML = '<i class="ti ti-alert-triangle"></i> Operator activity data missing for <b>' +
          m.count + '</b> date(s): ' + shown + (m.count > 8 ? " …" : "");
      } else el.hidden = true;
    }).catch(function () {});
  }
  ...
  function kv(k, v) { return '<div class="oa-kv"><span>' + k + '</span><span>' + txt(v) + '</span></div>'; }
  function renderProfile(p) {
    ...
    $("oaTabProfile").innerHTML = vleNote +
      ...
      kv("Name", p.operator_name) + kv("Operator ID", p.session_operator_id) +
      kv("Model", p.model) + kv("Operator Code", p.operator_code) +
  ...
  function init() {
    ...
    $("oaFrom").addEventListener("change", function () { state.from = this.value; resetPages(); loadCurrentView(); });
    $("oaTo").addEventListener("change", function () { state.to = this.value; resetPages(); loadCurrentView(); });
