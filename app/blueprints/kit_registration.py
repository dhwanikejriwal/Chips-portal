...
def _backend():
    return f"{get_backend_base_url()}/kit-registration"
...
def chips_l1_done(kit_id):
    ...
    try:
        resp = http.patch(f"{_backend()}/{kit_id}/l1-done", headers=_headers(), timeout=10)
        return Response(
            resp.content,
...
def chips_l2_done(kit_id):
    if not session.get("access_token"):
        return jsonify({"success": False, "error": "Session expired. Please log in again."}), 401
    try:
        resp = http.patch(f"{_backend()}/{kit_id}/l2-done", headers=_headers(), timeout=10)
        return Response(
            resp.content,
            status=resp.status_code,
            content_type=resp.headers.get("Content-Type", "application/json"),
...
def export_excel():
    ...
    try:
        resp = http.get(f"{_backend()}/export-excel?ids={ids}", headers=_headers(), stream=True)
        if resp.status_code == 401:
            return redirect(url_for("auth.logout"))
